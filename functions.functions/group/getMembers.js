...
exports.main = async ({ body }, sendResponse) => {
  ...
  if (!groupId) {
    sendResponse({ body: { status: "error", error: "groupId is required" }, statusCode: 400 });
    return;
  }
  ...
  const associationsEndpoint = `https://api.hubapi.com/crm/v4/objects/groups/${groupId}/associations/contact`;
  const batchContactsEndpoint = "https://api.hubapi.com/crm/v3/objects/contacts/batch/read";

  const leaderAssociationId = 229;
  const memberAssociationId = 305;
